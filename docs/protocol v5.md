...
All universes begin from a minimal set of foundational ideas collectively referred to as the Seed. These include the initial identity structures and the two fundamental poles of universal importance:

1. the currently existing individual human, and  
2. all life, consciousness, and intelligence in the universe through time.

Every identity created thereafter is linked to these foundational ideas according to the rules specified later in this protocol. The Seed defines the initial condition of the universe and provides the reference anchors required for deterministic reconstruction and universal-importance reasoning.
...
No institution, state, corporation, tribe, or identity MAY obtain unilateral control over the protocol, over node operation, or over any mechanism of truth, importance, or action determination. Stability, verification, and consensus MUST arise from the distributed operation of independent nodes. Governance MAY guide evolution but MAY NOT centralize authority. Any structural change that would introduce centralized control MUST be invalid under this protocol, even if a majority of contemporary identities attempt to approve it.
...
4. **Separation of validity and credit**
   ...
   - Such outcomes MAY affect importance, POD accumulation, and future eligibility according to active rulebooks.
...
The overlay anchor key is normatively defined as (scope_kind, anchor_id). For scope_kind = universal, anchor_id MUST reference the rulebook-defined universal reference anchor. For scope_kind = tribe, anchor_id MUST reference the tribe anchor idea. For scope_kind = personal, anchor_id MUST reference the owning identity idea.
...
The protocol defines two orthogonal axes of description structure: **length tiers** and **complexity tiers**. Length tiers specify how much text the description contains and include: a **sentence** description providing a minimal statement of the idea; a **paragraph** description offering a fuller human-readable formulation; and a **full** description capable of expressing extended detail. Complexity tiers specify the level of epistemic and linguistic constraint applied to the description: **fundamental**, **standard**, **advanced**, and **canonical**. Fundamental and standard descriptions use natural language and prioritize clarity for typical users. Advanced descriptions may include more technical or contextual detail. Canonical descriptions are tightly constrained, using a governed, minimal vocabulary defined elsewhere in the protocol to express the idea in a stripped-down, unambiguous form suitable for machine classification and cross-language interpretation.
...
Relative-importance connections express that idea A is important to or important for idea B. All importance relationships in the canonical universe, including universal importance and tribe importance, are implemented as relative_importance connections. Personal importance judgments MAY exist as non-canonical local overlays maintained by an identity for private reasoning or organization. Personal importance judgments become part of the canonical universe only if the identity explicitly publishes them as canonical relative_importance connections or as arguments within canonical importance challenges, at which point they are publicly visible and challengeable.
...
Offline or partitioned systems MAY record events locally but MUST NOT advance universal canonical cycles.
...
Offline operation is therefore equivalent to delayed publication, not parallel progression.
...
Epistemic reliability expressed through POD persists across the full ledger history. When claims are upheld or overturned years later, when predictions succeed or fail after long delays, when evidence accumulates or dissolves, POD flows gradually rebalance to reflect these outcomes. Because POD moves slowly and is tied to the full network of importance, it resists short-term volatility and captures reliability as a multi-year, multi-domain trajectory. No past contributions are erased; the system maintains the complete epistemic lineage of every identity, allowing reliability to emerge naturally from the structure of what the identity has said, how those claims performed, and how other identities judged their importance over time.
...
Personal scope denotes non-canonical or local judgments that do not affect the canonical universe unless explicitly published as canonical events.
...
The overall universal rank is a deterministic aggregation of the 20 axis ranks under the active rulebook set. The specific aggregation method is governed by the rulebook version in force at the applicable cycle boundary and MUST be applied consistently by all nodes.
...
Despite differences in visibility and eligibility, all scopes share identical semantics. Every scope has its own set of rank lists, defined entirely by relative importance connections and recorded importance-challenge verdicts. Universal, tribe, and personal importance are therefore three manifestations of the same underlying mechanism, distinguished only by who may see the lists, who may vote on them, and which reference ideas anchor the rankings.
...
A relative_importance connection does not create numeric scores or weights by itself. It places an idea into the ranking pool for the chosen reference idea, axis, timeframe, and scope. When two ideas share the same reference under the same scope and axis, they may be brought into a pairwise importance challenge. The winner moves immediately above the loser in that rank list, and through repeated challenges the rank list becomes an ordered pathway radiating outward from the reference idea. These rank lists are maintained separately for universal, tribe, and personal scopes, but all scopes use identical mechanics and semantics.

Interpretive significance pathways emerge from the global structure of relative-importance connections and the ordered rank lists they produce. Because relative importance edges express that one idea contributes to, derives meaning from, or matters for evaluating another, the graph acquires a readable structure in which more general or foundational ideas tend to anchor clusters of more specific, derivative, or action-oriented ideas. This interpretive structure is not encoded as a separate class of edges; it emerges entirely from relative_importance connections and importance-challenge outcomes. Universal-scope rankings are visible to everyone, and tribe- and personal-scope rankings appear as publicly readable overlays that show how particular communities or individuals prioritize the same public ideas relative to their chosen reference points.

Monotonic directionality is enforced **only for POD routing and derived POD-flow graphs**, not for the full set of relative_importance connections themselves. Universal importance provides a stable global gradient that constrains how POD may flow: POD MUST move from ideas with higher universal importance toward ideas with lower universal importance. POD MUST NOT flow upward against the universal-importance gradient. This rule ensures determinism, prevents reward cycles, and guarantees that POD flows from broad, foundational ideas toward more specific contributions and actions.
...
Importance challenges are the only way that relative-importance rankings change. They operate on the ranked list of ideas that share a reference idea, axis, timeframe, and scope (universal, tribe, or personal). A relative_importance connection places an idea into this list; an importance challenge proposes to move one idea higher in that list by comparing it directly to another idea that is currently ranked above it.
...
Epistemic events include idea creation, description proposals, argument and evidence submission, challenge openings, and selected-voter decisions. These events modify the epistemic state of the canonical graph directly and constitute the core of human participation in deliberation. As such, epistemic events are **direct POD sinks**. At each snapshot cycle, POD routed into an idea via universal and relative importance is deterministically allocated to the epistemic events that have modified that idea or materially contributed to its evaluated significance. Given the same event history and snapshot boundaries, all conformant nodes MUST reproduce the same POD assignments.

External completion events arise only from actionable ideas. POD routed into an actionable idea does not immediately transfer to any identity. Instead, it remains unassigned until a human identity completes the corresponding real-world action and submits a canonical completion truth claim. Only after the completion claim survives the full truth-challenge process does POD transfer from the actionable idea to the executing identity. External actions therefore function as **indirect POD sinks**, with assignment deferred until verification.
...
Action declarations and completion claims are canonical events and MUST be publicly visible and challengeable once published. Declarations MAY be omitted if rulebooks permit direct submission of a completion claim, but in all cases POD transfer requires a successful, challenge-resilient completion truth claim. Rulebooks MAY impose additional requirements on declarations or verification for specific classes of actionable ideas, but they MUST do so deterministically and without introducing non-replayable semantics.

Only after a completion truth claim withstands challenge does POD transfer from the actionable idea to the human identity that declared and completed the action. Until that point, POD routed into the actionable idea remains unassigned. If a completion claim is rejected, fails verification, or is superseded by a successful counter-challenge, the POD routed into that actionable idea for the relevant snapshot period MUST dissipate or remain unassigned according to the active token rulebooks. POD is never retroactively reassigned for failed or false completion claims.
...
Universal-importance shifts alter the POD-routing landscape for future cycles. When the importance of high-level ideas changes due to challenges, new evidence, or model updates, subsequent POD emissions follow new pathways. Events associated with ideas whose importance has diminished receive proportionally less POD in later cycles, while events connected to newly important ideas receive more. This rebalancing ensures that earlier POD allocations do not permanently dominate and that influence reflects continuing alignment with collective deliberation.

POD routed into actionable ideas or epistemic events dissipates in a controlled manner when it is not claimed. POD routed into actionable ideas that are never completed MUST dissipate or remain unassigned at snapshot completion, according to the active token rulebooks. POD routed into epistemic events that later lose relevance due to importance updates does not retroactively change past allocations, but becomes proportionally less influential as new POD enters circulation. No retroactive reassignment occurs.
...
- If an identity performs chain-maintenance work (e.g., producing a valid snapshot, serving archival packs, publishing commitments, or performing verification actions), the identity MAY earn POD only if the work is represented as canonical actions and truth claims, and those claims are not later disconfirmed or adjudicated as fraudulent.
...
Upon verdict, the protocol applies the domain-specific state update. This transformation modifies only the present canonical universe and SHALL NOT retroactively alter past ledger events. Truth-challenge verdicts update certainty or claim status (and may trigger POD transfer or governance-rule activation). Importance-challenge verdicts update ranking order. Action-challenge verdicts update which actionable ideas are endorsed or coordinated. Representation-challenge verdicts update identity relations or canonical description pointers. The transformation completes the challenge lifecycle.
...
The protocol defines a small number of identity-terminal events to preserve determinism when a real human dies, disappears, or formally transfers stewardship. A USER identity MAY issue a "succession declaration" as an actionable idea if they wish to predefine who inherits their POD, POINT, and verification lineage. This succession plan becomes active only if a later completion claim asserts that the real person has died or permanently ceased participation. The completion claim is a truth claim subject to challenge and evidence rules, requiring verifiable proof such as death certificates or trusted third-party attestations.
...
Offline seedpacks allow identities and groups to continue reasoning, creating ideas, conducting challenges, and drafting governance proposals while disconnected from the network. Offline operation is intended to mirror online semantics as closely as possible: the same idea types, challenge lifecycles, importance structures, and governance concepts apply. The primary distinction is not *what* can be done offline, but *what counts as canonical* when offline activity is later reintegrated.
...
Offline replicas MUST treat the governance rulebooks active at the moment of disconnection as authoritative for all local deliberation, including governance, safety, identity, token, and protocol rules. All offline activity is interpreted under those rulebooks unless and until reintegration occurs. Offline communities MAY continue to create canonical-structured ideas, arguments, challenges, and votes locally, but such activity remains non-canonical until explicitly published into the shared canonical universe.

Governance proposals MAY be drafted offline, argued offline, and even voted on offline, but offline verdicts SHALL NOT be considered canonical. Offline voting outcomes represent local consensus or preference only. Only verdicts that appear in the global canonical event log MAY activate future rulebooks or alter canonical governance behavior.
...
2. **Publish local work as canonical proposals or arguments.**  
   Offline ideas, challenges, arguments, and governance proposals MAY be published into the canonical universe as canonical events. Offline argumentation history MAY also be published as canonical arguments or evidence. Offline votes and verdicts SHALL be treated as ordinary opinions or arguments, not as canonical decisions.
...
* private or tribe-scoped offline ideas (legacy wording; tribe-scoped offline ideas as a separate canonical class is DEPRECATED and MUST be interpreted as offline-local drafts or overlay-intent until canonical publication).
...
Each idea MAY include multiple descriptions as separate **representation objects**, each with:

* **tier_length** - one of: `sentence`, `paragraph`, `full`,  
* **tier_complexity** - one of: `fundamental`, `standard`, `advanced`, `canonical`,  
* **payload_hash** - deterministic hash pointer to the canonical text payload bytes,  
* **authorship_identity** - identity providing this description.

Deterministic replay SHALL select the canonical description using rulebook-defined selection functions and canonical representation pointers.
