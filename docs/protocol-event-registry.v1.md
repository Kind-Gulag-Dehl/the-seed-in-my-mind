...
| `ordering_create` | A | Appendix A A4.2B.1 | Protocol v5 | Verified human canonical writer | Ordering object | Creates one authored Ordering under a declared profile | `ordering_id`, `ordering_profile`, `speaker_identity_id`, `item_idea_ids`, profile-conditional `vine_type` | A7 | Native Ordering conformance vectors; backend event-log validation | Open-core validator supports | Profiles are `vine`, `evidence_rail`, and `action_rail`; does not create graph connections. |
| `ordering_fork` | A | Appendix A A4.2B.2 | Protocol v5 | Verified human canonical writer | New Ordering from base Ordering | Creates full ordered replacement Ordering | `base_ordering_id`, `ordering_id`, `ordering_profile`, `speaker_identity_id`, `item_idea_ids`, profile-conditional `vine_type` | A7 | Native Ordering conformance vectors; backend event-log validation | Open-core validator supports | Declared profile MUST equal the base profile; authored history is preserved. |
