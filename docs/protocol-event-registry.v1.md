...
Count in this table: 38 Appendix A catalog event names, counting split aliases such as
...
| --- | --- | --- | --- | --- | --- | --- | --- | --- | --- | --- | --- |
| `identity_create` | A | Appendix A A4.1.1 | Profile-v0 identity admission, authorship, and encoding | Eligible human sponsor; applicant is neither author nor speaker | Event-derived `CanonicalAdmittedIdentity`, initial direct key, explicit structural roots, sponsor lineage, one capacity debit | Atomically creates `identity_kind = human`, active initial key, Mindgarden/Backyard of Relationships/Self Tree/Anthill, three root membership connections, and restricted initial lanes; no ordinary authority | `identity_id`, `initial_key_descriptor`, `initial_public_key_ref`, `admission_profile_version`, `capacity_period_id`, `rulebook_reference`, `admission_authorization_reference`, optional `verification_reference`, root plan, applicant proof | A7.2.4 admission/key errors, including `speaker_not_permitted`, proof, duplicate, authorization, capacity, and root errors | Future Profile-v0 admission vectors required; no vector artifact added in R2B | Specification reconciled; runtime Profile-v0 support not implemented | `speaker_identity_id` MUST be absent. Sponsor is envelope author; applicant proof binds final applicant-relevant fields. Existing validator behavior requiring target speaker/title is runtime drift, not protocol support. |
| `identity_verification_update` | Compatibility-only A | Appendix A A4.1.2 | Genesis/import/legacy compatibility | Explicit versioned compatibility manifest only | Historical compatibility record | Records legacy provenance/status without directly setting VH, VI, or any current eligibility | `identity_id`, provenance class, compatibility manifest ref, legacy status, optional legacy ref | `compatibility_event_not_authorized` and A7.2.4 | Future compatibility vectors required | Specification classification only; no ordinary runtime support | Not an ordinary post-genesis public event. It must not fabricate truth/evidence, sponsor, proof, debit, lineage, or authority. |
...
| `identity_key_rotate` | A | Appendix A A4.1.4 | Profile-v0 direct key lifecycle and authorship profile | Same human identity using its sole active direct key; no ordinary-writer requirement | Direct key state | Atomically supersedes current active key and activates validated replacement at finalized position | `identity_id`, `replacement_key_descriptor`, `replacement_public_key_ref`, `replacement_key_possession_proof` | `key_rotation_authorization_invalid`, `key_already_superseded`, `replacement_key_proof_invalid`, `public_key_already_registered` | Future rotation vectors required | Specification reconciled; runtime support not implemented | Envelope `public_key_ref` is the authorization key; `speaker_identity_id` and redundant `authorization_public_key_ref` are forbidden. Historical signatures remain valid. |
| `identity_key_revoke` | A | Appendix A A4.1.4 | Profile-v0 direct key lifecycle and authorship profile | Same human identity using its sole active direct key; no ordinary-writer requirement | Direct key state | Revokes a previously superseded key at finalized position; never retroactively invalidates history | `identity_id`, `revoked_public_key_ref` | `key_rotation_authorization_invalid`, `key_already_revoked`, `last_active_key_revocation_forbidden` | Future revocation vectors required | Specification reconciled; runtime support not implemented | Profile v0 has no recovery event and forbids revoking the sole active key. The narrow valid use is recording later compromise of a superseded key. |
...
| `idea_update_representation` | Historical-only; not live A | Appendix A A4.2.3 | Explicit versioned historical manifest only | No ordinary author | Legacy representation record | Preserves exact authorized history only; creates no new ordinary event path | Exact manifest-bound legacy payload | `unsupported_event_type` at ordinary ingress; invalid manifest binding in historical verification | Native Ordering/representation alias rejection vectors | Current clean-profile validator rejects | Excluded from the live catalog and live catalog count; never an alias accepted by ordinary ingress. |
...
| `representation_create` | A | Appendix A A4.2.5 | Protocol v5 | Verified human canonical writer; author equals event speaker and already exists | Candidate title or description representation object | Adds candidate representation; does not select it | Common: `representation_id`, `target_kind`, `target_object_id`, `representation_kind`, `payload_hash`, `author_identity_id`. `title` forbids all tier/vocabulary fields. `description` requires `tier_length` and `tier_complexity`; canonical complexity also requires `vocabulary_version_id` | A7, `invalid_payload_hash`, `invalid_field`, missing reference/author | Seed conformance-binding vectors; backend event-log/replay/import tests | Implemented and guarded-PostgreSQL verified; 19-case migration-0025 matrix passes | One title slot is separate from the twelve description cells. Canonical vocabulary references are explicit ordinary idea IDs, never defaults; pointer selection happens through `challenge_finalize_verdict`. |
| `ordering_create` | A | Appendix A A4.2B.1 | Protocol v5 | Verified human canonical writer | Ordering object | Creates one authored Ordering under a declared profile | `ordering_id`, `ordering_profile`, `speaker_identity_id`, `item_idea_ids`; Vine requires `vine_type`; standardized profiles require typed `subject_idea_id`, unique item IDs, and aligned `item_roles` | A7, subject/type/role/profile errors | Native Ordering conformance vectors; backend event-log/replay/import tests | Implemented and guarded-PostgreSQL verified; 19-case migration-0025 matrix passes | Evidence/Action Rails are standardized profiles; an Action Rail is one homogeneous potential or proposed lane, never both. |
| `ordering_fork` | A | Appendix A A4.2B.2 | Protocol v5 | Verified human canonical writer | New Ordering from base Ordering | Creates full ordered replacement Ordering | `base_ordering_id`, `ordering_id`, `ordering_profile`, `speaker_identity_id`, `item_idea_ids`; standardized profiles repeat subject and aligned roles; Vine may inherit `vine_type` | A7, subject/type/role/profile/lineage errors | Native Ordering conformance vectors; backend event-log/replay/import tests | Implemented and guarded-PostgreSQL verified; 19-case migration-0025 matrix passes | Fork preserves profile, subject, retained-item roles, and Action Rail lane; authored history remains append-only. |
...
| `challenge_create` | A | Appendix A A4.4.1 | Challenge Engine | Ordinary challenge-eligible verified human | Challenge object | Creates draft challenge | `challenge_id`, `challenge_domain`, `framing_representation_ref`, conditional `subject_idea_ids`/`subject_ordering_ids`; importance challenges additionally require `importance_context`, `rank_kind`, challenger, target, and the universal- or relative-context fields; optional future `tempo_lane` | A7, complete-context duplicate challenge rules | Tempo fixtures; backend tests | Open-core validator/storage support | The reference/universal pole is context, not a contestant. Private individual ordering is not a canonical challenge. Evidence-placement and certainty-band challenges remain scoped `challenge_create` uses, not separate event types. |
...
| Universal/relative rank outputs | Canonical graph, complete-context challenge outcomes, replay profile | Protocol/Replay | No | No | Snapshot derived outputs; API read surfaces | Existing replay/snapshot docs | Universal-axis and reference-relative outputs are distinct. `universal_position_sum`, exact mean, and `overall_universal_rank` derive from the twenty universal axis lists. Rank outputs are excluded from `state_root_hash` where specified. |
...
| `tempo_attestation_cast` | Removed specialized Tempo event; attestations are ideas in roles. | Ordinary ideas plus `connection_create` and challenge/verdict orderings. | Prior planning/spec drafts and harness guard. | Removed; harness rejects if seen. |
...
| `identity_verified` | Stale alias, not Appendix A canonical. | No ordinary Profile-v0 event; verification derives through ordinary truth/evidence/challenge procedures. | Older Protocol v5 prose. | `identity_verification_update` is compatibility-only, not the replacement canonical verification path. |
